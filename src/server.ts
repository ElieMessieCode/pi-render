...
app.get("/sse", (req: Request, res: Response) => {
  ...
  res.flushHeaders();
  sseClients.push(res);
  logger.debug(`Client SSE connecté (visuels) — total: ${sseClients.length}`, "sse");
  req.on("close", () => {
    sseClients = sseClients.filter(c => c !== res);
    logger.debug(`Client SSE déconnecté (visuels) — total: ${sseClients.length}`, "sse");
  });
});
...
app.get("/sse/debug", (req: Request, res: Response) => {
  ...
  res.flushHeaders();
  addDebugClient(res);
  logger.debug("Client SSE connecté (debug)", "sse");
  req.on("close", () => {
    removeDebugClient(res);
    logger.debug("Client SSE déconnecté (debug)", "sse");
  });
});
...
export function broadcastVisuals(): void {
  ...
  sseClients.forEach(c => { try { c.write(p); } catch (e) {
    logger.warn("Impossible d'écrire sur un client SSE visuel", "sse");
    sseClients = sseClients.filter(x => x !== c);
  }});
}

// ── API — Ajouter un visuel ───────────────────────────────────────────────────
app.post("/add", (req: Request, res: Response) => {
  try {
    const v = req.body as Visual;
    if (!v || !v.id || !v.content) {
      logger.warn("POST /add : payload invalide", "server", JSON.stringify(req.body).slice(0, 200));
      res.status(400).json({ ok: false, error: "payload invalide" });
      return;
    }
    history.push(v);
    broadcastVisuals();
    logger.info(`Visuel ajouté : "${v.title}" (${history.length} au total)`, "server");
    res.json({ ok: true });
  } catch (err) {
    logger.error("POST /add : erreur inattendue", "server", err);
...
app.get("/api/history", (_: Request, res: Response) => {
  try {
    ...
    logger.error("GET /api/history : erreur", "server", err);
...
app.get("/api/logs", (_: Request, res: Response) => {
  try {
    ...
    logger.error("GET /api/logs : erreur", "server", err);
    res.status(500).json({ error: "erreur interne" });
  }
});
...
app.post("/api/logs/clear", (_: Request, res: Response) => {
  try {
    logger.clear();
    // Notifie les clients debug via SSE que les logs ont été vidés
    // (le logger.clear() ne diffuse pas — on broadcast manuellement)
    const payload = `data: ${JSON.stringify({ type: "clear" })}\n\n`;
    // On accède aux clients via le module logger
    logger.info("Logs vidés par l'utilisateur", "server");
    res.json({ ok: true });
  } catch (err) {
    logger.error("POST /api/logs/clear : erreur", "server", err);
    res.status(500).json({ error: "erreur interne" });
  }
});
...
app.get("/", (_: Request, res: Response) => {
  ...
  catch (err) { logger.error("GET / : erreur", "server", err); res.status(500).send("Erreur interne"); }
});

app.get("/debug", (_: Request, res: Response) => {
  try { res.send(DEBUG_PAGE); }
  catch (err) { logger.error("GET /debug : erreur", "server", err); res.status(500).send("Erreur interne"); }
});
...
export function startServer(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const s = app.listen(PORT, () => {
      serverStarted  = true;
      serverInstance = s;
      logger.info(`Serveur démarré → http://localhost:${PORT}`, "server");
      logger.info(`Debug disponible → http://localhost:${PORT}/debug`, "server");
      resolve();
    });
    s.on("error", (e: NodeJS.ErrnoException) => {
      if (e.code === "EADDRINUSE") {
        logger.warn(`Port ${PORT} déjà utilisé — serveur supposé actif`, "server");
        serverStarted = true;
        resolve();
      } else {
        logger.fatal(`Impossible de démarrer le serveur : ${e.message}`, "server", e);
        reject(e);
      }
    });
    s.on("close", () => {
      logger.info("Serveur HTTP fermé", "server");
    });
  });
}

export function closeServer(): Promise<void> {
  logger.info("Fermeture du serveur en cours…", "server");
  return new Promise((resolve) => {
    // Ferme les SSE visuels
    sseClients.forEach(c => { try { c.end(); } catch {} });
    sseClients = [];
    logger.debug("Clients SSE visuels fermés", "server");

    if (!serverInstance) {
      logger.debug("Pas d'instance serveur active", "server");
      resolve();
      return;
    }
    ...
    const timeout = setTimeout(() => {
      logger.warn("Timeout fermeture serveur (2s) — forçage", "server");
      serverStarted  = false;
      serverInstance = null;
      resolve();
    }, 2000);
    ...
    serverInstance.close(() => {
      ...
      serverStarted  = false;
      serverInstance = null;
      logger.info("Serveur fermé proprement", "server");
      resolve();
    });
  });
}
...
function shutdown(signal: string): void {
  logger.warn(`Signal reçu : ${signal} — fermeture`, "process");
  closeServer().finally(() => {
...
html,body{min-height:100%;background:var(--bg);color:var(--ink);font-family:'Syne',sans-serif}
...
.btn{font-size:10px;font-weight:500;padding:3px 10px;border-radius:5px;background:transparent;border:0.5px solid var(--border);color:var(--muted);cursor:pointer;font-family:'Syne',sans-serif;transition:all .15s;display:flex;align-items:center;gap:4px;}
...
<header class="hdr">
  ...
  <div class="live"><div class="dot" id="dot"></div><span id="lbl">en direct</span></div>
  <a href="/debug" class="debug-link" target="_blank">🐛 debug</a>
  <div class="hdr-pill" id="cnt">0 visuel</div>
</header>
<div id="empty">
  <div class="ei">🎨</div>
  <h2>En attente de visuels…</h2>
  <p>Pose une question à pi-agent dans le terminal. Il générera des pages HTML interactives qui apparaîtront ici.</p>
...
function setLive(ok){document.getElementById('dot').className=ok?'dot':'dot off';document.getElementById('lbl').textContent=ok?'en direct':'reconnexion…';}
async function sync(){
...
function copySrc(id){const v=visuals.find(x=>x.id===id);if(!v)return;navigator.clipboard.writeText(v.content).then(()=>toast('✓ HTML copié'));}
